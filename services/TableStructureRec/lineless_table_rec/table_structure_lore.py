...
class TSRLore:
    ...
    def infer(self, input_content: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        slct_logi = self._run_process(slct_logi_feat, slct_dets_feat)
        return slct_output_dets, slct_logi

    def _run_process(
        self, logi_features: np.ndarray, det_features: np.ndarray
    ) -> np.ndarray:
        if not self.process_session.use_directml:
            _, logic = self.process_session(
                [logi_features, det_features.astype(np.int64)]
            )
            return logic

        cell_count = logi_features.shape[1]
        if cell_count == 0:
            return np.empty((1, 0, 4), dtype=np.float32)

        outputs = []
        for start in range(0, cell_count, self.PROCESS_BATCH_SIZE):
            stop = min(start + self.PROCESS_BATCH_SIZE, cell_count)
            chunk_size = stop - start
            logi_chunk = logi_features[:, start:stop]
            det_chunk = det_features[:, start:stop].astype(np.int64)

            # The exported LORE process graph declares a fixed 54-cell output.
            # DirectML enforces that shape even though CPU accepts shorter inputs.
            padding = self.PROCESS_BATCH_SIZE - chunk_size
            if padding:
                pad_width = ((0, 0), (0, padding), (0, 0))
                logi_chunk = np.pad(logi_chunk, pad_width, mode="edge")
                det_chunk = np.pad(det_chunk, pad_width, mode="edge")

            _, logic = self.process_session([logi_chunk, det_chunk])
            outputs.append(logic[:, :chunk_size])
