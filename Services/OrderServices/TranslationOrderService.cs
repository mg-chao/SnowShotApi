...
public interface ITranslationOrderService
{
    ...
    Task<UserTranslationOrder> CreateAsync(long userId, UserTranslationType type, string content, string from, string to, string domain, int promptTokens, int completionTokens);
    ...
    /// <returns>翻译订单</returns>
    Task<UserTranslationOrder?> UpdateAsync(long orderId, string? from = null, string? to = null, UserTranslationOrderStatus? status = null, int? promptTokens = null, int? completionTokens = null);
}
...
public class TranslationOrderService(ApplicationDbContext context, ITranslationOrderStatsService translationOrderStatsService) : ITranslationOrderService
{
    ...
    public async Task<UserTranslationOrder> CreateAsync(long userId, UserTranslationType type, string content, string from, string to, string domain, int promptTokens, int completionTokens)
    {
        ...
        await _translationOrderStatsService.UpdateAsync(userId, type, content.Length);

        return userTranslationOrder;
    }

    public async Task<UserTranslationOrder?> UpdateAsync(long orderId, string? from = null, string? to = null, UserTranslationOrderStatus? status = null, int? promptTokens = null, int? completionTokens = null)
    {
        ...
        {
            userTranslationOrder.PromptTokens = promptTokens.Value;
        }
        if (completionTokens != null)
        {
            userTranslationOrder.CompletionTokens = completionTokens.Value;
        }
